...
then
	...
	cp -r VMX_Bundle_Automation/ VMX_Bundle_Automation_1/
	cp -r VMX_Bundle_Automation/ VMX_Bundle_Automation_2/
	cd VMX_Bundle_Automation/
	...
	heat stack-create -f vmx_contrail.yaml -e vmx_contrail.env RC_VMX_1
	cd ..
	cd VMX_Bundle_Automation_1/
	var_n_name='final_test_network_3_'$dashed_project_uuid				
	sed -i 's/variable_network_name/'${var_n_name}'/' vmx_contrail.env
	sed -i 's/fixed_network_name/'${fixed_n_name}'/' vmx_contrail.env
	sed -i 's/_project_name_/'$1'/' vmx_contrail.env
	heat stack-create -f vmx_contrail.yaml -e vmx_contrail.env RC_VMX_2
	cd ..
	cd VMX_Bundle_Automation_2/
	var_n_name='final_test_network_4_'$dashed_project_uuid
	sed -i 's/variable_network_name/'${var_n_name}'/' vmx_contrail.env
	sed -i 's/fixed_network_name/'${fixed_n_name}'/' vmx_contrail.env
	sed -i 's/_project_name_/'$1'/' vmx_contrail.env
	heat stack-create -f vmx_contrail.yaml -e vmx_contrail.env RC_VMX_3
fi
